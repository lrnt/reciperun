...
export type AnnotatedRecipe = z.infer<typeof annotatedRecipeSchemaWithoutChecks>;
export const annotatedRecipeSchema = annotatedRecipeSchemaWithoutChecks.superRefine(
  checkAnnotationIngredientIndices
);

// Recipe with id
const recipeWithIdSchemaWithoutChecks = annotatedRecipeSchemaWithoutChecks.extend({
...
export const basicRecipeSchema = z.object({
  title: z.string().describe("Name of the recipe"),
  description: z.string().describe("Brief summary or introduction to the recipe"),
  ingredients: z.array(z.string()).describe("List of all ingredients needed for the recipe"),
  ...
  totalTime: z.number().describe("Total time in minutes (prep + cook)").optional(),
